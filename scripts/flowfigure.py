...
# LEFT SIDE: COUNTY TO FACILITY
county_shape.plot(ax = ax[0], color = "orange", alpha = 0.4, linewidth=1, edgecolor = "white")
# composters
...
for i in counties_popcen.index:
    ...
    for j in swis_df.index:
        ...
        if q > 1000000:
            _ = ax[0].plot([c_lon, f_lon], [c_lat, f_lat], 'g-', alpha = 0.6, linewidth=5.5)
        elif q > 200000:
            _ = ax[0].plot([c_lon, f_lon], [c_lat, f_lat], 'g-', alpha = 0.6, linewidth=3.5)
        elif q > 50000:
            _ = ax[0].plot([c_lon, f_lon], [c_lat, f_lat], 'g-', alpha = 0.6, linewidth=2.5)
        elif q > 20000:
            _ = ax[0].plot([c_lon, f_lon], [c_lat, f_lat], 'g-', alpha = 0.6, linewidth=1.5)
        elif q > 2000: 
            _ = ax[0].plot([c_lon, f_lon], [c_lat, f_lat], 'g-', alpha = 0.6, linewidth=0.75)
        elif q > 200: 
            _ = ax[0].plot([c_lon, f_lon], [c_lat, f_lat], 'g-', alpha = 0.6, linewidth=0.5)
...
ax[0].plot([],[], 'g-', alpha = 0.6, label = "Feedstock Flow")
...
# RIGHT SIDE
# california shape
county_shape.plot(ax = ax[1], color = "orange", alpha = 0.3, linewidth=1, edgecolor = "white")
# composters
scatter = ax[1].scatter(swis_df['lon'], swis_df['lat'], s = swis_df['cap_m3']/5000, 
    label = 'Compost Facility', color = 'dimgrey')
# Plot rangelands
rangelands.plot(ax = ax[1], color = 'white', alpha = 0.5, label = 'Rangeland')
for j in swis_df.index:
    f_no = swis_df['SwisNo'].iloc[swis_df.index == j].values[0]
...
# LEGEND FLOW - single bar
ax[1].plot([],[], 'b-', alpha = 0.6, label = 'Compost Flow')
