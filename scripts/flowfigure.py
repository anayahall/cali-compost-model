...
for j in swis_df.index:
	...
	for r in rangelands.index:
		...
		# q = c2fmatrix[i, j] # USE THIS FOR TEST MATRIX ONLY
			if q > 100000:
				_ = ax.plot([f_lon, r_lon], [f_lat, r_lat], 'm-', alpha = 0.6, linewidth=4.5)
			elif q > 600000:
				_ = ax.plot([f_lon, r_lon], [f_lat, r_lat], 'm-', alpha = 0.6, linewidth=3.5)
			elif q > 25000:
				_ = ax.plot([f_lon, r_lon], [f_lat, r_lat], 'm-', alpha = 0.6, linewidth=2.5)
			elif q > 10000:
				_ = ax.plot([f_lon, r_lon], [f_lat, r_lat], 'm-', alpha = 0.6, linewidth=1.5)
			elif q > 2000: 
				_ = ax.plot([f_lon, r_lon], [f_lat, r_lat], 'm-', alpha = 0.6, linewidth=0.75)
			elif q > 200: 
				_ = ax.plot([f_lon, r_lon], [f_lat, r_lat], 'm-', alpha = 0.6, linewidth=0.5)
